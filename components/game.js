...
const MoleGame = () => {
  ...
  const gifImageUri =
    "https://media1.giphy.com/media/l3vR1tookIhM8nZJu/giphy.gif?cid=ecf05e472i45sfi0af5g4ga6cnv3rgtke2gfv21bb9nb1bqq&ep=v1_gifs_search&rid=giphy.gif&ct=g"; // GIF 이미지 URL
  ...
  const moleImageUri =
    "https://i.namu.wiki/i/aJtwJmF0Ece9P0cM6dEahMsHi76985s26uZY4fAY-ROVpyGH2eGsAVR09PfNZeygyToACJJl97M-_wYr5bzNyVivyPcuirmUJuguUEJJfG0el3DGDSxlxWAoLkCSR9-P6ArWIeb1RwWl2Ni_D875CQ.webp"; // 두더지 이미지 경로
  const bombImageUri =
    "https://cdn.011st.com/11dims/resize/600x600/quality/75/11src/product/5321793482/B.jpg?499000000"; // 폭탄 이미지 경로
  const specialBlockImageUri =
    "https://static.wikia.nocookie.net/pokemon/images/1/13/%EB%8B%A5%ED%8A%B8%EB%A6%AC%EC%98%A4_%EA%B3%B5%EC%8B%9D_%EC%9D%BC%EB%9F%AC%EC%8A%A4%ED%8A%B8.png/revision/latest?cb=20170405005841&path-prefix=ko"; // 3점짜리 블록 이미지 경로
  ...
  useEffect(() => {
    ...
    const specialBlockInterval = setInterval(() => {
      const nextSpecialBlock = Math.floor(
  ...
  const gifPosition = timerLeft.interpolate({
    inputRange: [0, GAME_DURATION],
    outputRange: [-25, gaugeContainerWidth - 25], // GIF 이미지 크기의 절반만큼 보정
    extrapolate: "clamp",
  });
    ...
    <View style={styles.container}>
      {gameStarted && (
        <>
          <View style={styles.timeGaugeContainer}>
            <Animated.View
              style={[styles.timeGauge, { width: timeGaugeWidth, right: 0 }]}
            />
          </View>
          <Animated.Image
          ...
          <Text style={styles.timerText}>
            남은 시간: {Math.round(timeLeft / 1000)}초
          </Text>
      ...
      <View style={[styles.grid, { width: gridWidth, height: gridWidth }]}>
        {[...Array(GRID_SIZE * GRID_SIZE)].map((_, index) => (
          ...
          >
            {activeMole === index && gameActive && (
              <Image source={{ uri: moleImageUri }} style={styles.moleImage} />
            )}
            ...
            {activeSpecialBlock === index && gameActive && (
              <Image
                source={{ uri: specialBlockImageUri }}
      ...
      {!gameActive && (
        <Button
          title={gameStarted ? "게임 재시작" : "게임 시작"}
          onPress={gameStarted ? restartGame : startGame}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  grid: {
    flexDirection: "row",
    flexWrap: "wrap", // width: 600, // 격자 크기 조정
  },
  moleHole: {
    borderWidth: 1,
    borderColor: "black",
