...
import {Animated, Button, Dimensions, Easing, StyleSheet, Text, TextInput, TouchableOpacity, View} from 'react-native';
import Svg, {G, Path, Polygon, Text as SvgText} from 'react-native-svg';


const windowWidth = Dimensions.get('window').width;
const randName = ["철수", "영희", "민수", "준호", "지은", "혜진", "태형", "지훈", "유나", "민지"]
const RouletteGame = ({cnt}) => {
    ...
    const [name, setName] = useState(Array(cnt).fill(null).map((_, i) => `${randName[i]}`));
    useEffect(() => {
        // cnt 값이 변경될 때 name 배열 업데이트
        setName(Array(cnt).fill(null).map((_, i) => name[i] || `${randName[i]}`));
    }, [cnt]);
    ...
    const spinWheel = () => {
        ...
        Animated.timing(spinAnim, {
            toValue: randomSpin, duration: 4000, easing: Easing.out(Easing.ease), useNativeDriver: true,
        }).start(() => {
    ...
    for (let i = 0; i < cnt; i++) {
        ...
        sectors.push(<Path
            key={i}
            d={`M0 0 L${x1} ${y1} A${radius} ${radius} 0 ${angle > 180 ? 1 : 0} 1 ${x2} ${y2} Z`}
    ...
    return (<View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
        ...
        <Text style={{marginTop: 20}}>{`ㅋㅋㅋㅋㅋㅋ ${name[selectedIndex]}`}</Text>

        {/* 섹터 이름 편집 부분 */}
        <View style={{flexDirection: 'row'}}>
            {name.map((n, index) => (<View key={index} style={{flex: 1, alignItems: 'center'}}>
