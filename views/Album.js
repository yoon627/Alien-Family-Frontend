...
import {View, Text, StyleSheet, TouchableOpacity, Button, Pressable, Platform, ActionSheetIOS} from "react-native";
...
export default function Album({navigation}) {
        ...
        try {
            ...
            if (!cameraStatus?.granted) {
                ...
                if (result.assets && result.assets.length > 0) {
                    ...
                    setImageUrl(uri);
                    setImage(result.assets[0]);     // 이미지 정보를 image 상태에 저장 (s3 업로드용)
                    console.log(imageUrl);
                    uploadToServer(); // 이미지 선택 후 서버로 업로드
    ...
    const uploadToServer = async () => {
        ...
        const body = {
            filename: "IMG_7555.jpg",
            prefix: "2",
        };

        try {
            // 1단계: 서버에 presigned url 요청
            const urlRes = await fetch('/s3', {
                method: 'POST',
